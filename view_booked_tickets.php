...
<html>
	...
	<body>
		...
		<?php
			...
			$query="SELECT pnr,date_of_reservation,railway_no,journey_date,class,booking_status,no_of_passengers,payment_id FROM Ticket_Details where customer_id=? AND journey_date>=? AND booking_status='CONFIRMED' ORDER BY  journey_date";
			$stmt=mysqli_prepare($dbc,$query);
			mysqli_stmt_bind_param($stmt,"ss",$customer_id,$todays_date);
			mysqli_stmt_execute($stmt);
			mysqli_stmt_bind_result($stmt,$pnr,$date_of_reservation,$railway_no,$journey_date,$class,$booking_status,$no_of_passengers,$payment_id);
			...
			{
				echo "<table cellpadding=\"10\" style=\"margin-left: 280px\">";
				echo "<tr><th>PNR</th>
			...
			$query="SELECT pnr,date_of_reservation,railway_no,journey_date,class,booking_status,no_of_passengers,payment_id FROM Ticket_Details where customer_id=? and journey_date<? and journey_date>=? ORDER BY  journey_date";
			$stmt=mysqli_prepare($dbc,$query);
